...
MuJoCoROS::MuJoCoROS(const std::string &xmlLocation) : Node("mujoco_node")
{
    ...
    {
        const char *joint_name_ptr = mj_id2name(_model, mjOBJ_JOINT, i);
        // mj_id2name returns nullptr if the index is out of range (e.g. when _nq > _njnt) or if the
        // joint has no name. Creating an std::string from a nullptr is undefined-behaviour and can
        // seg-fault, so we fall back to a generated name in that case.
        if (joint_name_ptr)
        {
            _jointStateMessage.name[i] = joint_name_ptr;
        }
        else
        {
            _jointStateMessage.name[i] = "joint_" + std::to_string(i);
        }
    }
   ...
   // Create joint state publisher and joint command subscriber
    ...
    mjv_defaultScene(&_scene);                  // Ensure structure is zero-initialised before use
